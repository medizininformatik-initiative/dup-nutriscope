...
# Function to create histogram for Albumin and Phosphat
create_lab_histogram <- function(data, column_name, output_folder, file_name, title) {
  plot <- ggplot(data, aes(x = !!sym(column_name))) +
    geom_histogram(binwidth = 0.5, fill = "blue", color = "black", alpha = 0.7) +
    labs(title = paste("Verteilung", title), x = title, y = "Häufigkeit") +
    theme_minimal()
  
  # Save plot as PNG
  ggsave(file.path(output_folder, paste0(file_name, "_Histogramm_", title, ".png")), plot = plot, width = 8, height = 6)
}
